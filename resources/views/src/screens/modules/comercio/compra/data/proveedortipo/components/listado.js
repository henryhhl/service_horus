...
function C_ListadoProveedorTipo( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomerciocompraproveedortipo_index, {
            search: search, orderBy: 'asc', esPaginado: 0,
        } ) . then( (result) => {
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.proveedortipo );
            }
        } );
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idproveedortipo) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoProveedorTipo.propTypes = {
    value:  PropTypes.any,
...
C_ListadoProveedorTipo.defaultProps = {
    value: null,
    create: false,
}

export default C_ListadoProveedorTipo;
