...
function C_ListadoConceptoCompra( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomerciocompraconceptocompra_index, {
            search: search, orderBy: 'asc', esPaginado: 0,
        } ) . then( (result) => {
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.conceptocompra );
            }
        } );
    };
    ...
    function checkedData(item) {
        ...
        if (item.idconceptocompra == value) {
            item.checked = true;
        }
        return item.checked;
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idconceptocompra) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoConceptoCompra.propTypes = {
    value:  PropTypes.any,
...
C_ListadoConceptoCompra.defaultProps = {
    value: null,
    create: false,
}

export default C_ListadoConceptoCompra;
