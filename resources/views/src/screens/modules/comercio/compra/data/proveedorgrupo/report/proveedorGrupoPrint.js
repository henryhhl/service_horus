...
class ProveedorGrupoPrint extends Component {
    constructor( props ) {
        super( props );
        this.state = {};
    };
    render() {
        const { proveedorGrupo } = this.props;
        return (
            <>
                <ReactToPrint
                    trigger={ () => <a style={{ display: 'none', }} id="proveedorgrupo_print">Generar</a>}
                    content={ () => this.componentProveedorGrupoRef }
                >
                </ReactToPrint>
                <div style={{ display: 'none' }}>
                    <ProveedorGrupoComponent
                        proveedorGrupo={proveedorGrupo} 
                        ref={ el => (this.componentProveedorGrupoRef = el) }
                    />
                </div>
            </>
        );
...
class ProveedorGrupoComponent extends Component {
                            ...
                            <th colSpan='2' style={{ fontSize: 10, paddingTop: 5, color: 'grey', }}>
                                <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', }}>
                                    ...
                                    <div style={{ paddingRight: 40, display: 'block' }}>
                                        <div> { proveedorGrupo.fecha }</div>
                                        <div style={{ paddingLeft: 5, }}> { proveedorGrupo.hora }</div>
                                        {/* <div>Pag: 001</div> */}
                                    </div>
                                </div>
                            </th>
...
ProveedorGrupoPrint.propTypes = {
    proveedorGrupo: PropTypes.object,
};

ProveedorGrupoPrint.defaultProps = {
};

export default ProveedorGrupoPrint;
