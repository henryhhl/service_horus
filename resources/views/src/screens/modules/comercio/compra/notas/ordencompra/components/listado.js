...
function C_ListadoOrdenCompra( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomerciocompraordencompra_index, {
            ...
            if ( result.response == 1 ) {
                ...
                setArrayData( result.ordencompra );
            }
        } );
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idordencompra) ? {background: '#e0f3ff'} : {};
                                    return (
