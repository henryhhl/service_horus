...
function C_ListadoSolicitudCompra( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomerciocomprasolicitudcompra_index, {
            search: search, orderBy: 'asc', esPaginado: 0, isordencompra: props.isordencompra,
        } ) . then( (result) => {
            console.log(result)
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.solicitudcompra );
            }
        } );
    };
    ...
    function checkedData(item) {
        ...
        if (item.idsolicitudcompra == value) {
            item.checked = true;
        }
        return item.checked;
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idsolicitudcompra) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoSolicitudCompra.propTypes = {
    value:  PropTypes.any,
...
C_ListadoSolicitudCompra.defaultProps = {
    value: null,
    create: false,
    isordencompra: false,
}

export default C_ListadoSolicitudCompra;
