...
function C_ListadoAlmacen( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomercioinventarioalmacen_index, {
            search: search, orderBy: 'asc', esPaginado: 0,
            fkidsucursal: props.fkidsucursal,
        } ) . then( (result) => {
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.almacen );
            }
        } );
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idalmacen) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoAlmacen.defaultProps = {
    value: null,
    fkidsucursal: null,
    create: false,
}

export default C_ListadoAlmacen;
