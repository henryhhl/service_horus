...
function C_ListadoProductoSubGrupo( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomercioinventarioproductosubgrupo_index, {
            search: search, orderBy: 'asc', esPaginado: 0, fkidproductogrupo: props.fkidproductogrupo,
        } ) . then( (result) => {
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.productosubgrupo );
            }
        } );
    };
    ...
    function checkedData(item) {
        ...
        if (item.idproductosubgrupo == value) {
            item.checked = true;
        }
        return item.checked;
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idproductosubgrupo) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoProductoSubGrupo.propTypes = {
    value:  PropTypes.any,
...
C_ListadoProductoSubGrupo.defaultProps = {
    value: null,
    create: false,
    fkidproductogrupo: null,
}

export default C_ListadoProductoSubGrupo;
