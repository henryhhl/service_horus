...
function C_ListadoSeccionInventario( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomercioinventarioseccioninventario_index, {
            search: search, orderBy: 'asc', esPaginado: 0,
        } ) . then( (result) => {
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.seccioninventario );
            }
        } );
    };
    ...
    function checkedData(item) {
        ...
        if (item.idseccioninventario == value) {
            item.checked = true;
        }
        return item.checked;
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idseccioninventario) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoSeccionInventario.propTypes = {
    value:  PropTypes.any,
...
C_ListadoSeccionInventario.defaultProps = {
    value: null,
    create: false,
}

export default C_ListadoSeccionInventario;
