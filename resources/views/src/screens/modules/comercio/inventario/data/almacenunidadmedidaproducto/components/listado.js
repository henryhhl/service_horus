...
function C_ListadoAlmacenUnidadMedidaProducto( props ) {
    const { value } = props;
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomercioinventarioalmacenunidadmedidaproducto_index, {
            ...
            if ( result.response == 1 ) {
                ...
                setArrayData( result.arrayAlmacenUnidadMedidaProducto );
            }
        } );
    };
    ...
    function checkedData(item) {
        item.checked = false;
        if (item.idalmacenunidadmedidaproducto == value) {
            item.checked = true;
        }
        return item.checked;
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = ( value == item.idalmacenunidadmedidaproducto) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoAlmacenUnidadMedidaProducto.propTypes = {
    value:       PropTypes.any,
...
C_ListadoAlmacenUnidadMedidaProducto.defaultProps = {
    value: null,
    isventa: 'T',
    estado: null,
    fkidalmacen: null,
...
export default C_ListadoAlmacenUnidadMedidaProducto;
