...
function C_ListadoProductoMarca( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomercioinventarioproductomarca_index, {
            search: search, orderBy: 'asc', esPaginado: 0,
        } ) . then( (result) => {
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.productomarca );
            }
        } );
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idproductomarca) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoProductoMarca.propTypes = {
    value:  PropTypes.any,
...
C_ListadoProductoMarca.defaultProps = {
    value: null,
    create: false,
}

export default C_ListadoProductoMarca;
