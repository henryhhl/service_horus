...
class ConceptoInventarioPrint extends Component {
    constructor( props ) {
        super( props );
        this.state = {};
    };
    render() {
        const { conceptoInventario } = this.props;
        return (
            <>
                <ReactToPrint
                    trigger={ () => <a style={{ display: 'none', }} id="conceptoinventario_print">Generar</a>}
                    content={ () => this.componentConceptoInventarioRef }
                >
                </ReactToPrint>
                <div style={{ display: 'none' }}>
                    <ConceptoInventarioComponent
                        conceptoInventario={conceptoInventario} 
                        ref={ el => (this.componentConceptoInventarioRef = el) }
                    />
                </div>
            </>
        );
...
class ConceptoInventarioComponent extends Component {
                            ...
                            <th colSpan='2' style={{ fontSize: 10, paddingTop: 5, color: 'grey', }}>
                                <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', }}>
                                    ...
                                    <div style={{ paddingRight: 40, display: 'block' }}>
                                        <div> { conceptoInventario.fecha }</div>
                                        <div style={{ paddingLeft: 5, }}> { conceptoInventario.hora }</div>
                                        {/* <div>Pag: 001</div> */}
                                    </div>
                                </div>
                            </th>
...
ConceptoInventarioPrint.propTypes = {
    conceptoInventario: PropTypes.object,
};

ConceptoInventarioPrint.defaultProps = {
};

export default ConceptoInventarioPrint;
