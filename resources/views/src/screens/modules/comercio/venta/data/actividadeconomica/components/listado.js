...
function C_ListadoActividadEconomica( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomercioventaactividadeconomica_index, {
            search: search, orderBy: 'asc', esPaginado: 0,
        } ) . then( (result) => {
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.arrayActividadEconomica );
            }
        } );
    };
    ...
    function checkedData(item) {
        ...
        if (item.idactividadeconomica == value) {
            item.checked = true;
        }
        return item.checked;
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idactividadeconomica) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoActividadEconomica.propTypes = {
    value:  PropTypes.any,
...
C_ListadoActividadEconomica.defaultProps = {
    value: null,
    create: false,
}

export default C_ListadoActividadEconomica;
