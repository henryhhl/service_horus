...
function ComisionVentaPDF( props ) {

    const { conceptoVento } = props;
    const [ loading, setLoading ] = useState( false );

    return (
        <PDFViewer style={ styles.init } >
            <Document title="REPORTE COMISIÓN VENTA" onRender={ ( ) => {
                setLoading( true );
            } }>
                <Page size="A4" style={ styles.page }>
                    ...
                    { conceptoVento.arrayConceptoVenta.map( ( item, key ) => {
                        return (
...
ComisionVentaPDF.propTypes = {
    conceptoVento: PropTypes.object,

    visible: PropTypes.bool,
};

ComisionVentaPDF.defaultProps = {
    visible: false,
};

export default ComisionVentaPDF;
