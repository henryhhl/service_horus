...
class DevolucionNotaVentaPrint extends Component {
    constructor( props ) {
        super( props );
        this.state = {};
    };
    render() {
        const { devolucionNotaVenta } = this.props;
        return (
            <>
                <ReactToPrint
                    trigger={ () => <a style={{ display: 'none', }} id="devolucionnotaventa_print">Generar</a>}
                    content={ () => this.componentDevolucionNotaVentaRef }
                >
                </ReactToPrint>
                <div style={{ display: 'none' }}>
                    <DevolucionNotaVentaComponent
                        devolucionNotaVenta={devolucionNotaVenta}
                        ref={ el => (this.componentDevolucionNotaVentaRef = el) }
                    />
                </div>
            </>
        );
...
class DevolucionNotaVentaComponent extends Component {
                            ...
                            <th colSpan='9' style={{ fontSize: 10, paddingTop: 5, color: 'grey', }}>
                                <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', }}>
                                    ...
                                    <div style={{ paddingRight: 40, display: 'block' }}>
                                        <div> { devolucionNotaVenta.fecha }</div>
                                        <div style={{ paddingLeft: 5, }}> { devolucionNotaVenta.hora }</div>
                                        {/* <div>Pag: 001</div> */}
                                    </div>
                                </div>
                            </th>
...
DevolucionNotaVentaPrint.propTypes = {
    devolucionNotaVenta: PropTypes.object,
};

DevolucionNotaVentaPrint.defaultProps = {
};

export default DevolucionNotaVentaPrint;
