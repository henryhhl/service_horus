...
const NotaVentaXLSX = ( props ) => {
    ...
    const DataSet = [
        {
            ...
            data: notaVenta.arrayNotaVenta.map( ( element, key ) => [
    ...
    return (
        <ExcelFile
            filename="notaventa"
            element={
                <button type="button" id="buttonnotaventa_xlsx" style={ {display: 'none', } }
                >
                    Export Data
                </button>
            }
        >
            <ExcelSheet dataSet={DataSet} name="NOATA VENTA"/>
        </ExcelFile>
    );

};

NotaVentaXLSX.propTypes = {
    notaVenta: PropTypes.object,
};

NotaVentaXLSX.defaultProps = {
};

export default NotaVentaXLSX;
