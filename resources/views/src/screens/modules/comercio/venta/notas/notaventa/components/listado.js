...
function C_ListadoNotaVenta( props ) {
    ...
    function get_data( search = "" ) {
        httpRequest( 'get', webservices.wscomercioventanotaventa_index, {
            search: search, orderBy: 'asc', esPaginado: 0,
        } ) . then( (result) => {
            resultData( result );
            if ( result.response == 1 ) {
                C_Message( "success", "Servicio realizado exitosamente." );
                setArrayData( result.arrayNotaVenta );
            }
        } );
    };
                        ...
                        <table className="align-middle text-truncate mb-0 table table-bordered table-striped table-hover table-fixed">
                            ...
                            <tbody>
                                { array_data.map( ( item, key ) => {
                                    let style = (item.checked || value == item.idnotaventa) ? {background: '#e0f3ff'} : {};
                                    return (
...
C_ListadoNotaVenta.defaultProps = {
    value: null,
    create: false,
}

export default C_ListadoNotaVenta;
